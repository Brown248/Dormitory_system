...
import { fetchDormRooms, updateDormRoom, rolloverDormRooms, resetDormRooms } from "@/lib/api";
...
export default function DormitoryPage() {
            ...
            <table class="table">
              ...
              <tbody>
                ${currentRooms.map(room => {
                  ...
                  const wPrev = room.waterMeter || 0;
                  const wCurr = room.waterMeter ? (room.waterMeter + (room.waterCost ? Math.round(room.waterCost / currentWaterRate) : 0)) : 0;
                  const wCost = room.waterCost || 0;

                  const ePrev = room.electricityMeter || 0;
                  const eCurr = room.electricityMeter ? (room.electricityMeter + (room.electricCost ? Math.round(room.electricCost / currentElectricRate) : 0)) : 0;
  ...
  const handleOpenRoomModal = (room: Room) => {
    ...
    setPrevWater((room.waterMeter || 0).toString());
    setCurrWater(""); 
    setPrevElectric((room.electricityMeter || 0).toString());
    setCurrElectric(""); 
    setCleaningFee((room.cleaningFee || "").toString());
    setOtherFee((room.otherFee || "").toString());
    setLateDays((room.lateDays || "").toString());
    setPaymentStatus(room.paymentStatus || "pending");
            ...
            <div className="bg-white rounded-3xl p-6 lg:p-8 max-w-4xl w-full border border-slate-100 shadow-2xl relative my-auto max-h-[95vh] lg:max-h-[90vh] overflow-y-auto lg:overflow-visible">
              ...
              <form onSubmit={handleGenerateInvoiceSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
                ...
                <div className="lg:col-span-7 space-y-4">
                  ...
                  {/* section 1: ผู้เช่า & ค่าเช่า */}
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-3">
                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-wide block">👤 ข้อมูลผู้เช่า & ค่าเช่าห้องพัก</span>
                  ...
                  {/* section 3: ค่าใช้จ่ายเพิ่มเติม & วันชำระ */}
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-3">
                ...
                <div className="lg:col-span-5 flex flex-col justify-between bg-slate-50/50 p-5 rounded-2xl border border-slate-200/80 h-full min-h-[420px] lg:min-h-full">
                  ...
                  <div className="flex flex-col gap-2.5 pt-4 border-t border-slate-200 mt-4 lg:mt-auto">
                    <button
                      type="submit"
