  ...
  const links = [
    {
      ...
      items: [
        { href: "/", label: "Dashboard (ภาพรวม)", icon: (
    ...
    },
    {
      ...
      items: [
        { href: "/dormitory", label: "หอพัก (Dormitory)", icon: (
        ...
        { href: "/garage", label: "อู่ซ่อมรถ (Garage)", icon: (
        ...
        { href: "/house", label: "บ้านเช่า (Rental House)", icon: (
    ...
    },
    {
      ...
      items: [
        { href: "/invoices", label: "ประวัติธุรกรรม (Statement)", icon: (
          <svg className="w-5 h-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>
        )},
        { href: "/transactions", label: "บัญชี/การเงิน (Accounting)", icon: (
        ...
        { href: "/reports", label: "รายงานวิเคราะห์ (Reports)", icon: (
          <svg className="w-5 h-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
        )},
        { href: "/settings", label: "ตั้งค่า & ระบบ (Settings)", icon: (
    ...
    <>
      {links.map((group) => (
        <div key={group.section} className="space-y-1.5">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-6 px-3 first:mt-0">
            {group.section}
          </div>
          {group.items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold text-sm transition-colors relative ${
                  isActive
      ...
      <div className="mt-6 pt-4 border-t border-slate-100">
        <button
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold text-sm text-rose-500 hover:bg-rose-50 hover:text-rose-700 transition-colors w-full"
        >
